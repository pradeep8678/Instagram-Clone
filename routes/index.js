...
router.get("/profile", isLoggedIn, async function (req, res) {             //passport humne isliye use kara taaki user baar baar login na kare , he must hold
  let user = await userModel
    .findOne({ username: req.session.passport.user })
    .populate("posts")
    .populate("saved");
...
router.get("/profile/:user", isLoggedIn, async function (req, res) {
  let user = await userModel.findOne({ username: req.session.passport.user });  //same as above for the profile section
